...
const PongGame: React.FC<Props> = ({ player }) => {
  ...
  const [paddleB, setPaddleB] = useState({ y: 95, height: 30, width: 5, x: 285 });
  ...
  const updateGameLogic = () => {
    ...
    if (updatedBall.y + updatedBall.dy > 200 || updatedBall.y + updatedBall.dy < 0) {
    ...
    if (updatedBall.x - 10 < paddleA.width + 10 && updatedBall.y > paddleA.y && updatedBall.y < paddleA.y + paddleA.height) {
      updatedBall.dx = -updatedBall.dx;
    }

    if (updatedBall.x + 10 > paddleB.x && updatedBall.y > paddleB.y && updatedBall.y < paddleB.y + paddleB.height) {
      updatedBall.dx = -updatedBall.dx;
    }
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.font = "20px Arial";
      ctx.fillText(`Player A: ${score.playerA} | Player B: ${score.playerB}`, 50, 20);
  ...
  return (
    <div>
      <canvas ref={canvasRef} width="300" height="200" style={{ background: 'black' }}></canvas>
